...
TEST_F(MinHashTest, Permuted)
{
  ...
  // clang-format off
  LCW32 expected({
    {1392101586u,  394869177u,  811528444u},
    { 211415830u,  187088503u,  130291444u},
    {2098117052u,  394869177u,  799753544u},
    {2264583304u, 2920538364u, 3576493424u},
    { 253327882u,   41747273u,  302030804u},
    {2109809594u, 1017470651u,  326988172u},
    {1303819864u,  850676747u,  147107852u},
    { 736021564u,  720812292u, 1405158760u},
    { 902780242u,  134064807u, 1613944636u},
    { 547084870u, 1748895564u,  656501844u}
  });
  // clang-format on
  ...
  // clang-format off
  LCW64 expected64({
    { 827364888116975697ul, 1601854279692781452ul,  70500662054893256ul},
    {  18312093741021833ul,  133793446674258329ul,  21974512489226198ul},
    {  22474244732520567ul, 1638811775655358395ul, 949306297364502264ul},
    {1332357434996402861ul, 2157346081260151330ul, 676491718310205848ul},
    {  65816830624808020ul,   43323600380520789ul,  63511816333816345ul},
    { 629657184954525200ul,   49741036507643002ul,  97466271004074331ul},
    { 301611977846331113ul,  101188874709594830ul,  97466271004074331ul},
    { 121498891461700668ul,  171065800427907402ul,  97466271004074331ul},
    {  54617739511834072ul,  231454301607238929ul,  97466271004074331ul},
    { 576418665851990314ul,  231454301607238929ul,  97466271004074331ul}
  });
  // clang-format on
...
TEST_F(MinHashTest, PermutedWide)
{
  ...
  // clang-format off
  LCW32 expected({
    {1731998032u,  315359380u, 3193688024u},
    {1293098788u, 2860992281u,  133918478u}
  });
  // clang-format on
  CUDF_TEST_EXPECT_COLUMNS_EQUAL(*results, expected);
  ...
  // clang-format off
   LCW64 expected64({
     {1818322427062143853ul, 641024893347719371ul, 1769570368846988848ul},
     {1389920339306667795ul, 421787002125838902ul, 1759496674158703968ul}
   });
  // clang-format on
  CUDF_TEST_EXPECT_COLUMNS_EQUAL(*results64, expected64);
}
...
TEST_F(MinHashTest, PermutedManyParameters)
{
  ...
  // clang-format off
  LCW32 expected({
    {1731998032u,  315359380u, 3193688024u, 1777049372u,  360410720u, 3238739364u, 1822100712u,  405462060u,
    ...
    {1293098788u, 2860992281u,  133918478u, 1701811971u, 3269705464u,  542631661u, 2110525154u, 3678418647u,
  ...
  // clang-format off
   LCW64 expected64({
     {1818322427062143853,  641024893347719371, 1769570368846988848, 592272835132564366,
     ...
     {1389920339306667795,  421787002125838902, 1759496674158703968,  791363336977875075,
...
TEST_F(MinHashTest, Ngrams)
{
  ...
  auto input =
    LCWS({{"The", "quick", "brown", "fox", "jumpéd", "over", "the", "lazy", "brown", "dog."},
          {"The", "quick", "brown", "fox", "jumpéd", "over", "the", "lazy", "", "dog."},
          {"short", "row"}});
  ...
  // clang-format off
  LCW32 expected({
    { 230924604u,   55492793u, 963436400u},
    { 230924604u,  367515795u, 963436400u},
    {2380648568u, 1330223236u, 279797904u}
  });
  // clang-format on
  CUDF_TEST_EXPECT_COLUMNS_EQUAL(*results, expected);
  ...
  // clang-format off
  LCW64 expected64({
    { 208926840193078200ul, 576399628675212695ul,  312927673584437419ul},
    { 677038498284219393ul, 326338087730412201ul,  298455901014050223ul},
    {1493265692486268500ul, 720255058049417768ul, 2253087432826260995ul}
  });
  // clang-format on
  CUDF_TEST_EXPECT_COLUMNS_EQUAL(*results64, expected64);
}
...
TEST_F(MinHashTest, NgramsWide)
{
  ...
  // clang-format off
  LCW32 expected({
    { 571536396u, 2346676954u, 4121817512u},
    { 571536396u, 2346676954u, 4121817512u}
  });
  // clang-format on
  CUDF_TEST_EXPECT_COLUMNS_EQUAL(*results, expected);
