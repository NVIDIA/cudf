...
struct compute_size_and_concatenate_fn {
  ...
  column_device_view const string_narep_dv;
  ...
  __device__ void operator()(size_type const idx) const noexcept
  {
    ...
    for (size_type str_idx = start_idx; str_idx < end_idx; ++str_idx) {
      ...
      if (!d_chars && (null_element && !string_narep_dv.is_valid(0))) {
        size_bytes = 0;
        break;
      }
      ...
      auto const d_str = null_element ? string_narep_dv.element<string_view>(0)
...
struct scalar_separator_fn {
  ...
  {
    return d_separator.element<string_view>(0);
  }
};
  ...
  __device__ bool operator()(size_type idx)
  {
    ...
    if (valid_output) {
      ...
      for (size_type str_idx = start_idx; str_idx < end_idx; ++str_idx) {
        ...
        if (!valid_element && !comp_fn.string_narep_dv.is_valid(0)) { return false; }
      }
                                           ...
                                           rmm::device_async_resource_ref mr)
{
  ...
  auto const string_narep_view = narep.as_column_view();
  auto const sep_dv = column_device_view::create(separator_view.as_column_view(), stream);
  auto const string_narep_dv =
    column_device_view::create(string_narep_view.as_column_view(), stream);
...
struct column_separators_fn {
  ...
  column_device_view const sep_narep_dv;

  [[nodiscard]] __device__ bool is_null_list(column_device_view const& lists_dv,
                                             size_type const idx) const noexcept
  {
    return lists_dv.is_null(idx) || (separators_dv.is_null(idx) && !sep_narep_dv.is_valid(0));
  }
  ...
  [[nodiscard]] __device__ string_view separator(size_type const idx) const noexcept
  {
    return separators_dv.is_valid(idx) ? separators_dv.element<string_view>(idx)
                                       : sep_narep_dv.element<string_view>(0);
  }
};
                                           ...
                                           rmm::device_async_resource_ref mr)
{
  ...
  auto const lists_dv_ptr      = column_device_view::create(lists_strings_column.parent(), stream);
  auto const strings_dv_ptr    = column_device_view::create(strings_col.parent(), stream);
  auto const string_narep_view = string_narep.as_column_view();
  auto const string_narep_dv =
    column_device_view::create(string_narep_view.as_column_view(), stream);
  auto const sep_dv_ptr     = column_device_view::create(separators.parent(), stream);
  auto const sep_narep_view = separator_narep.as_column_view();
  auto const sep_narep_dv   = column_device_view::create(sep_narep_view.as_column_view(), stream);
