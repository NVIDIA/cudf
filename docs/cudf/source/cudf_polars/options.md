...
| `max_concurrent_io_tasks` | Number of concurrent IO producer tasks for each scan node. Automatic by default. Use an integer for one limit, or a dict with `local` and/or `remote` keys to tune local and remote scans separately. The environment variable accepts an integer, `auto`, or a JSON dict. | auto        |
...
For the memory trade-offs of IO concurrency, see the [memory tuning guide](memory_errors.md#cudf-polars-memory-errors-concurrent-io).
