...
struct contains_scalar_dispatch {
                                                           ...
                                                           cuda::stream_ref stream) const
  {
    ...
    auto const needle_view = needle.as_column_view();
    auto const d_needle    = column_device_view::create(needle_view.as_column_view(), stream);
    ...
    return cudf::detail::count_if(
             ...
             [d_needle = *d_needle] __device__(auto const val_pair) {
               return val_pair.has_value() && (d_needle.element<DType>(0) == *val_pair);
             },
             stream) > 0;
  }
                                                          ...
                                                          cuda::stream_ref stream) const
  {
    ...
    auto const needle_view = needle.as_column_view();
    auto const needle_tv   = table_view{{needle_view.as_column_view()}};
...
bool contains(column_view const& haystack, scalar const& needle, cuda::stream_ref stream)
{
  ...
  if (not needle.is_valid()) { return haystack.has_nulls(); }
