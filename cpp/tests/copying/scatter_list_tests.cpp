...
TYPED_TEST(TypedScatterListsTest, SlicedInputListsOfLists)
{
  ...
  CUDF_TEST_EXPECT_COLUMNS_EQUIVALENT(
    cudf::test::lists_column_wrapper<T, int32_t>{{{2, 2}, {7, 7}},
                                                 {{6, 6}, {5, 5}, {4, 4}},
                                                 {{1, 1}, {8, 8}},
                                                 {{9, 9}, {8, 8}, {7, 7}},
                                                 {{6, 6}, {5, 5}, {4, 4}},
...
TYPED_TEST(TypedScatterListsTest, SlicedInputListsOfStructs)
{
  ...
  auto src_list_column =
    cudf::make_lists_column(3, offsets_column{0, 2, 4, 6}.release(), src_structs.release(), 0, {});
  auto src_sliced = cudf::slice(src_list_column->view(), {1, 3}).front();
  ...
  auto expected          = cudf::make_lists_column(
    5, offsets_column{0, 2, 3, 5, 6, 7}.release(), expected_structs.release(), 0, {});
...
TEST_F(ScatterListsTest, SlicedInputListsOfStrings)
{
  ...
  CUDF_TEST_EXPECT_COLUMNS_EQUIVALENT(
    cudf::test::lists_column_wrapper<cudf::string_view>{{"two", "two"},
                                                        {"b", "b", "b", "b", "b"},
                                                        {"one", "one", "one"},
                                                        {"d", "d", "d", "d", "d"},
                                                        {"e", "e", "e", "e", "e"}},
