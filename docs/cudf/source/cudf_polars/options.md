...
| `max_concurrent_io_tasks` | Number of concurrent IO producer tasks for each scan node. Automatic by default; see {class}`~cudf_polars.engine.options.StreamingOptions` for the current policy. Set explicitly to tune IO throughput versus memory pressure. | auto        |
...
For the memory trade-offs of IO concurrency, see {ref}`cudf-polars-memory-errors-concurrent-io`.
