...
template <typename Equal>
class hash_csr_pair_equal {
 public:
  explicit hash_csr_pair_equal(Equal check_row_equality)
    : _check_row_equality{std::move(check_row_equality)}
  {
  }
                                             ...
                                             probe_key_type const& rhs) const noexcept
  {
    using detail::row::lhs_index_type;
    using detail::row::rhs_index_type;
    return _check_row_equality(lhs_index_type{lhs.second}, rhs_index_type{rhs.second});
  }
...
class primitive_hash_csr_pair_equal {
 public:
  explicit primitive_hash_csr_pair_equal(
  ...
  Fn&& fn)
{
  ...
  if (cudf::detail::is_primitive_row_op_compatible(right_table)) {
    ...
    return std::forward<Fn>(fn)(primitive_hash_csr_pair_equal{d_equal}, d_hasher);
  }
  ...
  if (cudf::detail::has_nested_columns(left_table)) {
    ...
    return std::forward<Fn>(fn)(hash_csr_pair_equal{d_equal}, d_hasher);
  }
