...
| `max_concurrent_io_tasks` | Number of concurrent IO producer tasks for each scan node. Automatic by default. Tune with an integer or a `{"local": ..., "remote": ...}` dict. | auto        |
